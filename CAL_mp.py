'''
2023-11-26: Indrasen Bhattacharya
Notebook to implement constraint satisfaction approach
This uses some process-based parallelism to speed up computation
...
- write a new member function to allow for some array manipulations
...
def radonCalc( CALobj, real3D ):
    
    
    nTheta_loc = CALobj.nAngles
    angles_loc = CALobj.angles

    region_shape = np.shape(real3D)
    zSlices = region_shape[0]


    zArr = np.arange( zSlices )
    
    global perSliceRadon
    
    def perSliceRadon(iZ):
        return radon( real3D[iZ, ...], angles_loc, circle=True )

    with mp.Pool(8) as p:
        projections = list( p.map( perSliceRadon , zArr ) )

    return np.array(projections)

def reconstruction( CALobj, projections, reconFilter=None):
    ...
    zArr = np.arange( proj_shape[0] )
    
    global perSliceIradon
    
    def perSliceIradon(iZ):
        return iradon( projections[iZ, ...], theta=angles_loc, filter_name=reconFilter, circle=True )
    
    with mp.Pool(8) as p:
        region_3D_recon = list( p.map( perSliceIradon , zArr ) )

    #for iZ in range(proj_shape[0]):
    #    region_3D_recon[iZ,...] = iradon(projections[iZ,...], theta=angles_loc, 
    #                                     filter_name=reconFilter, circle=True)

    region_3D_recon = np.array(region_3D_recon) * factor
...
class CAL_mp():
    ...
    #private method to calculate projections over all the z slices
    '''
    def __radonCalc(self, real3D):
        ...
        #angles_loc = angles_loc * np.ones( (zSlices, ) )
        
        zArr = np.arange( zSlices )
        #maps the indices of zSlices over the slice by slice radon defined as a lambda function
        
        #projFunc = radon3D( real3D, angles_loc )
        
        with mp.Pool(8) as p:
            projections = list( p.map( lambda iZ: radon( real3D[iZ, ...], angles_loc, circle=True ) , zArr ) )
        
        
        
        #for iZ in range(zSlices):
            #projections[iZ,...] = radon_(real3D[iZ,...], angles_loc)
    ...
    def __plotProj(self, proj, iZ=0, saveName=None):
        ...
        recon = reconstruction(self, proj)
    ...
    def initializeProjections(self, displZ = None):
        ...
        projLoc = radonCalc(self, real3D=self.imgArr[...,0])
        angles = self.angles
        ...
        recon_None = reconstruction(self, projections=projLoc)
        recon_RL = reconstruction(self, projections=filtered_loc_rl)
        recon_SL = reconstruction(self, projections=filtered_loc_sl)
        ...
        recon_none_pos = reconstruction(self, projections=none_masked, reconFilter=None)
        recon_RL_pos = reconstruction(self, projections=rl_masked, reconFilter=None)
        recon_SL_pos = reconstruction(self, projections=sl_masked, reconFilter=None)
    ...
    def loss(self, proj_iter):
        ...
        dose_3D_iter = reconstruction(self, projections=proj_iter)
    ...
    def loss_gradient(self, proj_iter):
        ...
        gradient_iter = radonCalc(self, real3D=gradient_dose_iter)
    ...
    def displLoss(self, proj_iter, 
        ...
        result_recon = reconstruction(self, projections=proj_iter)
    ...
    def runOpt(self, options_loc=None, displZ=None):
        ...
        self.finalDose = reconstruction(self, projections=self.finalProj)
        ...
        saveDict['dose_3D_init_none'] = reconstruction(self, projections=self.none_masked)
        saveDict['dose_3D_init_SL'] = reconstruction(self, projections=self.sl_masked)
